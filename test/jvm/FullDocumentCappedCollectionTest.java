...
public class FullDocumentCappedCollectionTest extends OpLogTestBase {
    ...
    public void setUp() throws UnknownHostException {
        Mongo mongo = MongoUtils.getMongo();
        // Get db
    ...
    public void aggregateOpLogFieldAndUpdateDocumentInMongoDB() throws UnknownHostException {
        ...
        // Create a mongo bolt
        MongoUpdateBolt mongoSaveBolt = new MongoUpdateBolt("mongodb://test:test@mongo.test.server:20011/storm_mongospout_test", "stormoutputcollection", updateQuery, mapper, WriteConcern.NONE);
        // Set the spout
        builder.setSpout("mongodb", new MongoCappedCollectionSpout("mongodb://test:test@mongo.test.server:20011/storm_mongospout_test", "aggregation", query), 1);
        // Add a bolt
        ...
        while(!done) {
            ...
            if(result != null && ((Integer)result.get("sum")).intValue() >= 3675 * 4) {
                done = true;
            } else {
                try {
                    Thread.sleep(100);
    ...
    public void aggregateOpLogFieldAndInsertADocumentPrResultInMongoDB() throws UnknownHostException {
        ...
        // Create a mongo bolt
        MongoInsertBolt mongoInserBolt = new MongoInsertBolt("mongodb://test:test@mongo.test.server:20011/storm_mongospout_test", "stormoutputcollection", mapper, WriteConcern.NONE, false);
        // Set the spout
        builder.setSpout("mongodb", new MongoCappedCollectionSpout("mongodb://test:test@mongo.test.server:20011/storm_mongospout_test", "aggregation", query), 1);
        // Add a bolt
        builder.setBolt("sum", new FullDocumentCappedSummarizer(), 1).allGrouping("mongodb");
